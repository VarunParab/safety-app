...
class _SignUpState extends State<SignUp> {
  ...
  // Registration function that includes the UID logic and Firestore data storage
  registration() async {
    if (password != null) {
      try {
        // Create the user and retrieve the UserCredential
        UserCredential userCredential = await FirebaseAuth.instance
            .createUserWithEmailAndPassword(email: email, password: password);

        // Get the UID of the newly created user
        User? user = userCredential.user;
        String uid = user?.uid ?? '';  // Safe null-check for uid
        ...
        await FirebaseFirestore.instance.collection('users').doc(uid).set({
          ...
          'name': name,
          'email': email,
          'created_at': DateTime.now(),  // Store the timestamp when the account was created
        });
        ...
        // Navigate to Home and pass the UID (if needed for future actions)
        Navigator.push(
            context, MaterialPageRoute(builder: (context) => MainScreen(uid: uid)));
